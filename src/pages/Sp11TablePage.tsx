...
const Sp11TablePage = () => {
  ...
  const filteredSongs = useMemo(() => songs.filter(song => {
    ...
    if (filters.cleartype && filters.cleartype.length > 0 && !filters.cleartype.includes(lamp)) {
      return false;
    }
    if (filters.unlocked !== undefined && filters.unlocked !== unlocked) {
      return false;
    }
    ...
    if (filters.version && filters.version.length > 0) {
      const version = versionMap[song.id]?.version;
      if (!filters.version.includes(version)) return false;
    }
    if (filters.label && filters.label.length > 0) {
      const label = songInfo[song.id]?.label;
      if (!filters.label.includes(label)) return false;
    }

    return true;
  }), [songs, clearData, chartInfo, filters, songInfo, versionMap]);
